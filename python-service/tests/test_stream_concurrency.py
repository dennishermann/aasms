...
async def test_stream_relevance_concurrency():
    ...
    with patch(
        "src.services.inclusion_evaluation_service.InclusionEvaluationService"
    ) as MockServiceClass:
    ...
    # Verification: each source emits a "started" + "progress" event = 40 total
    assert len(results) == 40

    # Check that we received "progress" events for each source
