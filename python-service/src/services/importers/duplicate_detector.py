...
class DuplicateDetector:
    ...
    def find_duplicate(self, candidate: ParsedSource) -> Optional[Tuple[str, str]]:
        """
        ...
        if candidate.title and candidate.authors:
            ...
            for existing in self.existing_sources:
                ...
                existing_first = self._normalize_author(existing_authors[0]) if existing_authors else ""
                
                # High similarity threshold (0.9) to avoid false positives
                title_similarity = difflib.SequenceMatcher(None, normalized_title, existing_title).ratio()
