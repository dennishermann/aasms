...
class ImportOrchestrator:
    ...
    def get_importer(self, database_source: str) -> BaseImporter:
        ...
        if not importer:
            raise ValueError(
                f"Unsupported database source: {database_source}. "
                f"Supported: IEEE, ACM, SCOPUS"
            )
        return importer
    ...
    async def process_import(
        ...
        logger.info(
            "import_orchestrator: parsed",
            extra={"parsed_count": len(parsed_sources)}
        )
        ...
        if not skip_relevance_check:
            ...
            for i in range(0, len(new_sources), batch_size):
                ...
                logger.info(f"import_orchestrator: processing batch {i // batch_size + 1}, items {i} to {i + len(batch)}")
                ...
                for source in batch:
                    ...
                    source_content = {
                        ...
                        "venue": source.venue,
                        "publication_date": str(source.publication_date) if source.publication_date else "",
                        "content_excerpt": "", # No full text during import
                    }
                    tasks.append(checker.evaluate_inclusion(
                        source_content=source_content,
                        inclusion_criteria=[{'criterion': c} for c in inclusion_criteria], # format for service
        ...
        while True:
            ...
            if event.get("type") == "progress" or event.get("type") == "error":
                ...
                if event.get("type") == "progress":
                     print(f"DEBUG: import_orchestrator: task {event['index']} completed ({completed_count}/{total})")
