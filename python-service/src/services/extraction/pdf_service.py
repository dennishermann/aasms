...
class PdfMetadataService:
    ...
    async def extract_metadata(
        ...
        # Determine model
        if not model_name:
            model_name = "gpt-4o-mini" if self.provider_name == "openai" else "claude-3-haiku-20240307"
    ...
    async def _extract_claude(self, model_name: str, prompt: str, pdf_bytes: Optional[bytes]) -> Dict[str, Any]:
    ...
    async def _extract_openai(self, model_name: str, prompt: str, text_excerpt: str, pdf_bytes: Optional[bytes]) -> Dict[str, Any]:
        ...
        if pdf_bytes:
            ...
            uploaded = await client.files.create(file=("excerpt.pdf", pdf_bytes), purpose="assistants")
            file_id = uploaded.id
