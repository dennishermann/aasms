...
class ParsedSourceResult(BaseModel):
    ...
    url: str | None = None
    publication_date: str | None = None  # Added field
    ...
    try:
        ...
        for s in parsed_sources:
            ...
            res = ParsedSourceResult(
                ...
                year=int(s.publication_date)
                if s.publication_date
                and s.publication_date.isdigit()
                and len(s.publication_date) == 4
                else None,
            )
            results.append(res)
...
async def check_single_relevance(request: CheckRelevanceRequest):
    ...
    try:
        ...
        return RelevanceCheckResult(
            ...
            reasoning=(
                res.get("inclusion_reasoning", "") + " " + res.get("exclusion_reasoning", "")
            ).strip(),
        ...
        raise HTTPException(status_code=500, detail=f"Relevance check failed: {str(e)}") from e
...
async def process_import(
    ...
    try:
        ...
        result = await orchestrator.process_import(
            ...
            exclusion_criteria=study_params.get(
                "exclusion_criteria", []
        ...
        response = {
            ...
            "duplicates": [
                {"source": s.dict(), "existing_id": eid, "match_method": method}
            ...
            "new_sources": [
                {
                    **s.dict(),
                    ...
                    if not skip_relevance_check and idx in result["relevance_results"]
                    else None,
                }
