...
export function useParametersData(studyId: string): UseParametersDataResult {
    ...
    const { data: apiFacets, isLoading: facetsLoading, isRefetching: facetsFetching } = useQuery({
    ...
    const initialFormData = useMemo<ParametersFormData | null>(() => {
        ...
        return {
            ...
            inclusionCriteria: parameters ? parameters.inclusionCriteria.map(ic => ({
                criterion: ic.criterion,
                order: ic.order,
            })) : [],
            exclusionCriteria: parameters ? parameters.exclusionCriteria.map(ec => ({
