...
export async function POST(
    ...
    try {
        ...
        const inclusion_criteria = firstSource.study.parameters?.inclusionCriteria.map((ic) => ic.criterion) || [];
        const exclusion_criteria = firstSource.study.parameters?.exclusionCriteria.map((ic) => ic.criterion) || [];
        ...
        const pythonSourcesPayload = sources.map((s, idx) => {
            ...
            return {
                title: s.title,
                authors: s.authors,
                publication_date: s.publicationDate ? s.publicationDate.getFullYear().toString() : undefined,
        ...
        if (!pythonRes.ok) {
            ...
            return NextResponse.json({ error: "Python service failed", details: txt }, { status: pythonRes.status });
        }
