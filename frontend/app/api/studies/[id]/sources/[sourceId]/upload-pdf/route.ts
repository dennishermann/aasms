...
export async function POST(
  ...
  try {
    ...
    const suggestKeywordMapping = async (
      keyword: string,
      facet: (typeof source.study.facets)[number],
    ) => {
    ...
    const inclusionReasoning = skipInclusion
      ? existingAnalysis?.inclusionReasoning
      : analysisResult.inclusionReasoning;
    const exclusionReasoning = skipInclusion
      ? existingAnalysis?.exclusionReasoning
      : analysisResult.exclusionReasoning;
    const inclusionCriteria = skipInclusion
      ? existingAnalysis?.inclusionCriteria || []
      : analysisResult.inclusionCriteria;
    const exclusionCriteria = skipInclusion
      ? existingAnalysis?.exclusionCriteria || []
    ...
    await prisma.source.update({
      ...
      data: {
        ...
        decisionRationale: isIncluded
          ? null
          : analysisResult.exclusionReasoning || "Excluded by AI",
      },
    });
    ...
    return NextResponse.json(
      {
        error: "Failed to upload and analyze PDF",
