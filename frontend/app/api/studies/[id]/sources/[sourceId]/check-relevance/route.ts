...
export async function POST(
    ...
    try {
        ...
        const payload = {
            ...
            inclusion_criteria: source.study.parameters?.inclusionCriteria.map((ic) => ic.criterion) || [],
            exclusion_criteria: source.study.parameters?.exclusionCriteria.map((ic) => ic.criterion) || [],
        };
        ...
        return NextResponse.json(
            { error: "Failed to check relevance", details: error instanceof Error ? error.message : "Unknown" },
            { status: 500 }
        );
    }
}
