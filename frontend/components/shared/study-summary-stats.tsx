...
import { FileText, CheckCircle, XCircle, Clock, Calendar, Building } from "lucide-react";
...
export function StudySummaryStats({ stats, isLoading, variant = "full" }: StudySummaryStatsProps) {
  if (isLoading) {
    const skeletonCount = variant === "full" ? 6 : 4;
    return (
      <div
        className={`grid grid-cols-2 md:grid-cols-${variant === "full" ? 3 : 4} lg:grid-cols-${skeletonCount} gap-4`}
      >
  ...
  const gridCols =
    variant === "full" ? "grid-cols-2 md:grid-cols-3 lg:grid-cols-6" : "grid-cols-2 md:grid-cols-4";
