...
import { Download, ExternalLink, ChevronDown, ChevronUp, FileText, Calendar, Users, Building } from "lucide-react";
...
async function fetchSourcesByIds(
    ...
    const response = await fetch(
        `/api/studies/${studyId}/sources/by-ids?ids=${sourceIds.join(",")}`
    );
    if (!response.ok) {
...
export function SourceDrilldown({
    ...
    return (
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="!max-w-5xl !w-[90vw] max-h-[90vh] overflow-hidden flex flex-col" showCloseButton={false}>
