...
export function CrossTabTable({
  ...
  const rowData = useMemo<RowData[]>(() => {
    ...
    return data.rowLabels.map((row) => {
      const cellMap = new Map<string, { count: number; percentage: number; sourceIds: string[] }>();
  ...
  const columns = useMemo<ColumnDef<RowData>[]>(() => {
    ...
    const cols: ColumnDef<RowData>[] = [
      {
        ...
        cell: ({ row }) => <div className="font-medium">{row.original.rowLabel}</div>,
      },
    ];
    ...
    cols.push({
      ...
      cell: ({ row }) => (
        <div className="text-center font-semibold font-mono">{row.original.total}</div>
      ),
    });
            ...
            <TableBody>
              ...
              <TableRow className="bg-muted/50 font-semibold">
                ...
                {data.colLabels.map((col) => {
                  const colTotal = data.colTotals.find((t) => t.id === col.id)?.count ?? 0;
                  return (
