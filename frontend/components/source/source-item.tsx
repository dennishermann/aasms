        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            <Link href={getSourceUrl(source.id)} className="flex-1 min-w-0 mr-4 group">
              ...
              <p className="text-sm text-muted-foreground truncate">
                {source.authors?.join(", ")} • {source.venue || "Unknown Venue"} •{" "}
                {source.publicationDate ? new Date(source.publicationDate).getFullYear() : ""} •{" "}
                {source.status}
              </p>
            </Link>

            <div className="flex items-center gap-2 shrink-0">
              <Badge variant={source.status === "INCLUDED" ? "default" : "outline"}>
                {source.status}
              </Badge>
          ...
          {currentTab === "pending" && suggestion && (
            <div className="rounded-md border bg-background p-3 mt-2">
              ...
              <p className="text-sm">
                <strong>Authors:</strong> {(suggestion.authors || []).join(", ") || "—"}
              </p>
              <p className="text-sm">
                <strong>Venue:</strong> {suggestion.venue || "—"}
              </p>
            </div>
          )}
        </div>
