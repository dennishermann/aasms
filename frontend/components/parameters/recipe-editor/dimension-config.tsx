...
export function DimensionConfig({
  ...
  const filteredFacets = facets.filter((f) => allowedTypes.includes(f.type));
    ...
    <div className="space-y-2">
            ...
            <div className="px-2 py-4 text-sm text-muted-foreground text-center">
              ...
              {hasHiddenOpenFacets && (
                <p className="mt-2 text-amber-600">
                  {openFacets.length} OPEN facet{openFacets.length > 1 ? "s" : ""} need coding
      ...
      {hasHiddenOpenFacets && filteredFacets.length > 0 && (
        <p className="text-xs text-amber-600 flex items-center gap-1">
          ...
          {openFacets.length} OPEN facet{openFacets.length > 1 ? "s" : ""} hidden. Run &quot;Develop
          Coding&quot; in Analysis to convert them.
        </p>
      )}
