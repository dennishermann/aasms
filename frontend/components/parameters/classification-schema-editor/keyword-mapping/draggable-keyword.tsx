...
export function SourceIcon({ source }: { source?: string | null }) {
    if (source === "LLM") return <span title="AI-suggested"><Bot className="h-3 w-3 text-muted-foreground shrink-0" /></span>;
    if (source === "USER") return <span title="User-assigned"><User className="h-3 w-3 text-muted-foreground shrink-0" /></span>;
    return null;
}
