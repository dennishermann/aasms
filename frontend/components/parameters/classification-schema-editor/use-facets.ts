...
export function useFacets(
    ...
    const handleRemoveFacet = useCallback((facetKey: string) => {
        onChange(value.filter(f => getFacetKey(f) !== facetKey));
    }, [value, onChange, getFacetKey]);

    const handleUpdateFacet = useCallback((facetKey: string, updates: Partial<Facet>) => {
        onChange(value.map(f => getFacetKey(f) === facetKey ? { ...f, ...updates } : f));
    }, [value, onChange, getFacetKey]);

    const handleAddCategory = useCallback((facetKey: string) => {
    ...
    const handleRemoveCategory = useCallback((facetKey: string, categoryIndex: number) => {
        const facet = value.find(f => getFacetKey(f) === facetKey);
        if (facet) {
